...
```json
{
  "primaryTaskId": "CODE-001",
  "parallelTracks": ["CODE-002", "CODE-003"],
  "dependencyGraph": {
    "CODE-001": [],
    "CODE-002": [],
    "CODE-003": ["CODE-002"]
...
```bash
node scripts/sprint-board.mjs update --id CODE-001 --state "In Progress"
node scripts/sprint-board.mjs journal --task CODE-001 --summary "what changed"
```
...
4. When marking a task `Failed`, record a pitfall entry alongside the state update:
   ```bash
   node scripts/sprint-board.mjs update --id CODE-001 --state "Failed" \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..."
   node scripts/sprint-board.mjs pitfall --task CODE-001 \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..." \
     [--missing-context "..."]
   ```
